import { FileClock, FileText, LayoutDashboard, Landmark, LogOut, Receipt, Wallet, Zap } from "lucide-react";
...
export function AppSidebar({
  ...
  return (
    <aside
      className={cn(
        "hidden shrink-0 flex-col border-r border-slate-200 bg-white/96 py-4 text-slate-700 backdrop-blur md:flex",
        collapsed ? "w-[88px] px-3" : "w-[220px] px-3"
      )}
    >
      <div className={cn("mb-5", collapsed ? "px-0" : "px-1")}>
        <div
          className={cn(
            "rounded-2xl border border-slate-100 bg-white p-3 shadow-sm",
            collapsed ? "flex justify-center px-2 py-4" : "flex items-center gap-3"
          )}
        >
          ...
          {!collapsed ? (
            <div className="min-w-0">
              <p className="truncate text-[15px] font-bold tracking-tight text-sky-900">Debt Recovery</p>
              <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-400">Portal</p>
            </div>
          ) : null}
        </div>
      </div>
      ...
      <div className="px-1 pb-2">
        {!collapsed ? (
          <p className="mb-2 px-2 text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400">Main</p>
        ) : null}
      ...
      >
        {!collapsed ? (
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="truncate text-[13px] font-semibold text-slate-700">{user?.full_name}</p>
        ...
        <button
          ...
          className={cn(
            ...
            collapsed ? "justify-center p-2" : "items-center gap-2 px-2 py-2"
          )}
