...
export default function AdminDashboardPage() {
  ...
  async function handleExportBills() {
    if (exportStart && exportEnd && exportStart > exportEnd) { toast.error("Start date cannot be after end date."); return; }
    if ((billsQuery.data?.count ?? 0) === 0) { toast.error("No records to export."); return; }
    try {
      const p = { start_date: exportStart || undefined, end_date: exportEnd || undefined };
    ...
    <AppShell title="Admin Dashboard">
      <div className="space-y-6">
        <PageHeader
          ...
          actions={
            <>
              ...
              <Button variant="outline" onClick={() => void handleRebuild()} disabled={rebuildMutation.isPending}>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <KpiCard title="Total Collection" value={formatCurrency(kpis?.total_collection ?? 0)} icon={BadgeIndianRupee} accentClassName="bg-[#6F72BE]" />
          <KpiCard title="Total Payments" value={String(kpis?.total_payments ?? 0)} icon={Wallet} accentClassName="bg-[#22A55A]" />
          <KpiCard title="Cleared Bills" value={String(kpis?.total_cleared_bills ?? 0)} icon={ReceiptIndianRupee} accentClassName="bg-[#D97B0A]" />
          <KpiCard title="Cash" value={formatCurrency(kpis?.total_cash ?? 0)} icon={Wallet} accentClassName="bg-[#22A55A]" />
          <KpiCard title="UPI" value={formatCurrency(kpis?.total_upi ?? 0)} icon={Wallet} accentClassName="bg-[#6F72BE]" />
          <KpiCard title="Cheque" value={formatCurrency(kpis?.total_cheque ?? 0)} icon={ReceiptIndianRupee} accentClassName="bg-[#D97B0A]" />
          <KpiCard title="Electronic" value={formatCurrency(kpis?.total_electronic ?? 0)} icon={Wallet} accentClassName="bg-[#6F72BE]" />
        </div>
        ...
        <BillsTable
          ...
          onEdit={(bill) => { setEditingBill(bill); setIsBillModalOpen(true); }}
          ...
          filters={
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
              ...
              <div className="flex flex-col gap-1">
                ...
                <DateInput value={exportStart} onChange={setExportStart} clearable max={exportEnd || undefined} />
              </div>
              <div className="flex flex-col gap-1">
                <Label className="text-[11px] text-[#9898B4]">Export end</Label>
                <DateInput value={exportEnd} onChange={setExportEnd} clearable min={exportStart || undefined} />
              </div>
              <Button variant="outline" size="sm" onClick={() => { setExportStart(""); setExportEnd(""); }}>
                Reset
              </Button>
            </div>
          }
      ...
      <DeleteBillDialog
        ...
        onOpenChange={(open) => { if (!open) setDeleteBillId(null); }}
        billId={deleteBillId}
      />
    </AppShell>
  );
