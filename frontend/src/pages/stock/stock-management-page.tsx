    ...
    <AppShell>
      <div className="space-y-5">
        ...
        <div className="flex w-fit gap-1 rounded-[14px] border border-[#DFE1F0] bg-[#F6F7FC] p-1 shadow-[0_2px_8px_rgba(30,30,48,0.06)]">
          {TABS.map((tab) => (
            <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className={cn("rounded-[10px] px-4 py-1.5 text-[13px] font-semibold transition-all", activeTab === tab.id ? "bg-white text-[#6F72BE] shadow-[0_2px_8px_rgba(30,30,48,0.06)]" : "text-[#9898B4] hover:text-[#6F72BE]")}>
              {tab.label}
            </button>
          ))}
        </div>
        {activeTab === "levels" && <StockLevelsTab />}
        {activeTab === "movements" && <StockMovementsTab />}
        {activeTab === "transfers" && <StockTransfersTab />}
      </div>
    </AppShell>
  );
}