...
def import_bills_from_excel(file) -> dict:
    ...
    required_columns = ["invoice_number", "invoice_date", "route_name", "outlet_name", "brand", "actual_amount"]
    errors: list[dict] = []
    imported = 0
    ...
    for index, row in df.iterrows():
        ...
        try:
            ...
            outlet = Outlet.objects.filter(name=outlet_name, route__name=route_name).select_related("route").first()
            if not outlet:
